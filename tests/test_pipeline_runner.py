...
def test_pipeline_creates_artifacts(monkeypatch):
    ...
    # Verify predictions contain identifiers OR encoded features plus outcomes
    required_cols = [
        "predicted_outcome",
        "predicted_prob",
    ]
    # At least one identifier set must be present
    identifiers_sets = [
        {"homeTeam", "awayTeam"},
    ...
    assert any(ids.issubset(set(preds.columns)) for ids in identifiers_sets), (
        "Predictions must contain either team names (homeTeam/awayTeam) "
        "or encoded identifiers (home_team_encoded/away_team_encoded)"
    )
