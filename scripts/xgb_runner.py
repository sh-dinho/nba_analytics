...
def xgb_runner(data, todays_games_uo, frame_ml, games, home_team_odds, away_team_odds, kelly_criterion=True):
    """
    ...
    for i, (home_team, away_team) in enumerate(games):
        ...
        if home_team_odds[i] and away_team_odds[i]:
            ev_home = Expected_Value.expected_value(ml_preds[i][1], int(home_team_odds[i]))
            ev_away = Expected_Value.expected_value(ml_preds[i][0], int(away_team_odds[i]))

        # Kelly fractions
        kelly_home = kc.calculate_kelly_criterion(home_team_odds[i], ml_preds[i][1]) if kelly_criterion else None
        kelly_away = kc.calculate_kelly_criterion(away_team_odds[i], ml_preds[i][0]) if kelly_criterion else None
