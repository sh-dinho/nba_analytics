...
def main(model_type: str | None = None):
    ...
    try:
        ...
        after = len(data)
        if after < before:
            logger.warning(f"⚠️ Dropped {before - after} rows with missing labels")

        # Ensure we still have enough rows to train
        if len(data) < 3:
            raise PipelineError("Not enough labeled rows after cleaning to train the model.")

        # Cast label to int if it looks binary but float
        label_unique = set(np.unique(data["label"].dropna()))
        if label_unique.issubset({0, 1, 0.0, 1.0}):
            try:
        ...
        # Stratify only if both classes exist and counts allow
        stratify = y if len(set(y)) > 1 and min(y.value_counts()) >= 2 else None
