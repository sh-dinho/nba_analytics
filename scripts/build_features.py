...
def main(n_rounds=10):
    ...
    logger.info(f"Building training features with {n_rounds} rounds of matchups...")
    df = pd.read_csv(stats_file).fillna(0)

    team_stats = df.groupby("TEAM_ABBREVIATION").agg({
        "PTS": "mean", "AST": "mean", "REB": "mean", "STL": "mean", "BLK": "mean"
    }).reset_index()

    if len(team_stats) < 2:
        logger.error("Not enough teams to build matchups.")
        raise ValueError("Need at least 2 teams to create features.")
    ...
    for _ in range(n_rounds):
        ...
        for i in range(0, len(teams) - 1, 2):
            ...
            h = team_stats.loc[team_stats["TEAM_ABBREVIATION"] == home].iloc[0]
            a = team_stats.loc[team_stats["TEAM_ABBREVIATION"] == away].iloc[0]
            games.append({
                "game_id": game_id,
                "home_team": home, "away_team": away,
                "home_pts_avg": h["PTS"], "away_pts_avg": a["PTS"],
                "home_ast_avg": h["AST"], "away_ast_avg": a["AST"],
                "home_reb_avg": h["REB"], "away_reb_avg": a["REB"],
    ...
    if features_df["home_win"].nunique() == 1:
        logger.warning("Only one label found in home_win. Injecting diversity.")
    ...
    logger.info(f"Feature summary: Avg PTS diff={avg_pts_diff:.2f}, "
