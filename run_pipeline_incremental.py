# ============================================================
# File: run_pipeline_incremental.py
# Purpose: NBA daily pipeline with incremental historical updates
# ============================================================
...
def incremental_update(historical: pd.DataFrame) -> pd.DataFrame:
    """Fetch only missing games (incremental)."""
    new_data = []
    for season in SEASONS:
        ...
        if not season_df.empty and "startDateEastern" in season_df.columns:
            last_date = pd.to_datetime(season_df["startDateEastern"]).max()
        else:
            last_date = None
        ...
        if last_date:
            df_season["startDateEastern"] = pd.to_datetime(
                df_season["startDateEastern"]
            )
